...
P=[5,   100,      100,     30,      100,   1,      1e-3,   1e-4];
param = initParam(P);
% MTLF
...
[A,result]=optimization(C,w0',Xci,Xcj,param,sd_tra,td_tra,x_target);
%% Predict
...
%% optimization
function [At,result]=optimization(C,w0,Xci,Xcj,param,sd_tra,td_tra,x_target)

 sGraph=ConstructWeightedNNGraph(x_target,3);
    L=CalLaplacianMatrix(sGraph);
    clear sGraph;
    manifold_term=x_target'*L*x_target;
...
while convA>eplsion && iter<100
    ...
    for i=1:size(Xci,1)
        vij=Xci(i,:)-Xcj(i,:);
        sumA = sumA+A0*(1-vij'*vij)*pair_weights(i)*C(i,3)/((600)*(600-1));
    end
    At = At-gamma*(-beta*sumA+2*A0-10^-2  *manifold_term);
    %% updata omega
    zeta = zeros(ns+nt,1);
    for k=1:size(Xci,1)
