...
addpath('/Users/aravindchalla/Desktop/PIE');

disp('Load data...');

dataset = {'PIE05','PIE07','PIE09','PIE27','PIE29'};
%data = load('usps-mnist.mat');

src = load(char(dataset(1))); %load source domain data
dst = load(char(dataset(2)));% load target domain data

X_src=src.fea;%source domain feature
ns=size(X_src,1); %number of source domain samples
Y_src=src.gnd; %source domain label data

X_dst=dst.fea;% target domain feature
nt=size(X_dst,1); % number of target domain samples
Y_dst=dst.gnd; %source domain label data
data.data.train.source=X_src;
data.data.train.target=X_dst;
data.data.test.target=X_dst;
data.labels.train.source=Y_src';
data.labels.train.target=Y_dst';
data.labels.test.target=Y_dst';
...
% %  k,	C,      dim,	sigma,	lamda,	beta,	gamma,	gammaW
% P=[1,   100,      500,     30,      100,   1,      1e-3,   1e-0];
% param = initParam(P);
...
%% load data
X = [data.data.train.source];
y = [data.labels.train.source'];
Xtest = data.data.test.target;
ytest = data.labels.test.target';

X_target_data = [data.data.train.target];
X_target_label = [data.labels.train.target];
...
index = []
for i = 1:68
    yp = find(X_target_label==i);
    index = [yp(1:3)];
    for j = 1:3
        X_temp = [X_temp; X_target_data(index(j), 1:1024)];
        y_temp = [y_temp; X_target_label(1, index(j))];
...
sd_tra = X(1:ntra_s,:);
td_tra = Xtest;
[A,result]=optimization(C,w0',Xci,Xcj,param,sd_tra,td_tra,Xtest);
save result
%% Predict
...
%% optimization
function [At,result]=optimization(C,w0,Xci,Xcj,param,sd_tra,td_tra,Xtest)
...
% tic
 ...
 ntt=size(Xtest,1)
 X_data=[sd_tra;Xtest];
ee=[1/nss * ones(nss,1);-1/ntt*ones(ntt,1)];
...
while convA>eplsion && iter<100
    ...
    At = At-gamma*(beta*sumA+2*A0+0*MMD);
    %% updata omega
    zeta = zeros(ns+nt,1);
    for k=1:size(Xci,1)
