...
addpath('C:\Users\rakesh\Desktop\newTML\ACM_TML_dataset\PIE');
...
src = load(char(dataset(4))); %load source domain data
dst = load(char(dataset(5)));% load target domain data
...
while convA>eplsion && iter<100
    ...
    At = At-gamma*(beta*sumA+2*A0+0.01*MMD);
    %% updata omega
    zeta = zeros(ns+nt,1);
    for k=1:size(Xci,1)
